...
def predict_and_store_one(image_path, true_label_one_hot, model, cnn_input_size=CNN_INPUT_SZ,
    ...
    for idx, patch in enumerate(patches):
        ...
        pred = model.predict(image_batch, verbose=0)#关掉预测进度条打印
...
if __name__ == '__main__':
    ...
    # 遍历 train_1 ~ train_9 文件夹，收集所有图片路径
    all_img_paths = []
    for i in range(1, 10):
        folder = f'../raw_data/train_{i}'
        all_img_paths.extend(glob.glob(os.path.join(folder, '*.jpg')))

    # 用于计数每个类别已采样数量
    style_count = {style: 0 for style in STYLE_MAPPING.keys()}
    max_per_class = 5000  # 每个类别最大采样数量

    # 只处理属于 STYLE_MAPPING 的类别，且每类不超过1600张
