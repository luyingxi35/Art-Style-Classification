...
PATCH_NUM = 2
CLASS_NUM = 5
TEST_DATA_PATH = 'resnet_test_data_2x5'  # 测试数据集路径
...
class ShallowNN(nn.Module):
    def __init__(self, input_dim=PATCH_NUM*CLASS_NUM, hidden_dim=128, hidden_dim1=64, output_dim=CLASS_NUM):
        super().__init__()
        self.flatten = nn.Flatten()  # 将5×5输入展平为25维
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.relu = nn.ReLU()
...
if __name__ == '__main__':
    ...
    if IS_BASELINE:
        ...
        model = ShallowNN_2(input_dim=PATCH_NUM*CLASS_NUM)
