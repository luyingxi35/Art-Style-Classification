...
class CustomDataset(Dataset):
    def __init__(self, folder_path):
        ...
        # 输入为 6×17 的张量
        self.inputs = [torch.tensor(item['scores'], dtype=torch.float32) for item in self.data]
        # 输出为单个 17 维向量
...
def load_and_test(model_path, input_sample):
    ...
    model = ShallowNN(input_dim=30, hidden_dim=128)
...
if __name__ == "__main__":
    ...
    # 加载数据
    train_dataset = CustomDataset('train_data_for_shallow')
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)

    # 初始化模型
    model = ShallowNN(input_dim=30, hidden_dim=HIDDEN_DIM)
    ...
    # 保存模型
    torch.save(model.state_dict(), 'shallow_nn_6x5.pth')