        ...
        for file_name in os.listdir(folder_path):
            if file_name.endswith('.json'):
                ...
                try:
                    ...
                    if (isinstance(scores, list) and len(scores) == PATCH_NUM and all(isinstance(row, list) and len(row) == VEC_DIM for row in scores)
                        and isinstance(label, list) and len(label) == VEC_DIM):
...
def load_and_test(model_path, input_sample):
    ...
    model = ShallowNN(input_dim=PATCH_NUM*VEC_DIM, hidden_dim=128)
...
if __name__ == "__main__":
    ...
    # 加载数据
    train_dataset = CustomDataset('1_train_data_for_shallow')
    ...
    # 保存模型
    torch.save(model.state_dict(), 'shallow_nn_1x17.pth')